...
import { Link, Table, TableCell, TableHeader, TableRow, Text } from 'blockchain-info-components'
  ...
  return (
    <div style={{ minWidth: '900px', paddingBottom: '45px' }}>
      <Table style={{ minWidth: '900px' }}>
        <TableHeader>
          <TableCell width='20%'>
            <Text size='12px' weight={500}>
              <FormattedMessage id='scenes.borrow.history.created' defaultMessage='Created' />
            </Text>
          </TableCell>
          <TableCell width='30%'>
            <Text size='12px' weight={500}>
          ...
          <TableCell width='15%'>
            <Text size='12px' weight={500}>
              <FormattedMessage id='scenes.borrow.history.collateral' defaultMessage='Collateral' />
            </Text>
          </TableCell>
          <TableCell width='12.5%'>
            <Text size='12px' weight={500}>
              <FormattedMessage id='scenes.borrow.history.amount' defaultMessage='Loan Amount' />
            </Text>
          </TableCell>
          <TableCell width='12.5%'>
            <Text size='12px' weight={500}>
        ...
        {props.borrowHistory
          .sort((a, b) => moment(b.openedAt).valueOf() - moment(a.openedAt).valueOf())
          .map((loan) => {
            ...
            return (
              <TableRow key={loan.loanId}>
                <TableCell width='20%'>
                  <Value data-e2e='loanCreationDate'>{moment(loan.openedAt).format('lll')}</Value>
                </TableCell>
                <CollateralizationTableCell data-e2e='loanStatus' width='30%'>
                  <Status {...loan} />
                  {offer && (
                    <CollateralizationBarWrapper data-e2e='collateralizationBar'>
                      <CollateralizationBar {...props} loan={loan} offer={offer} />
                ...
                <TableCell width='12.5%'>
                  ...
                  <Value>
                    <CoinDisplay coin='PAX'>
                      {loan.principal.amount.find((amount) => amount.currency === 'PAX')!.amount}
                    </CoinDisplay>
                  </Value>
                </TableCell>
                <TableCell width='12.5%'>
                  <Value data-e2e='outstandingInterest'>
                    {loan.financials ? (
                      loan.financials.owedInterest[0] ? (
                        <CoinDisplay coin={loan.financials.owedInterest[0].currency}>
