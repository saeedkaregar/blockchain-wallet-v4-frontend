...
export default ({ api, coreSagas, networks }) => {
  ...
  return function* sendXlmSaga() {
    ...
    yield takeLatest(AT.FIRST_STEP_SUBMIT_CLICKED, sendXlmSagas.firstStepSubmitClicked)
    yield takeLatest(AT.FIRST_STEP_MAXIMUM_AMOUNT_CLICKED, sendXlmSagas.maximumAmountClicked)
    yield takeLatest(AT.SECOND_STEP_SUBMIT_CLICKED, sendXlmSagas.secondStepSubmitClicked)
    yield takeLatest(
      // @ts-ignore
