...
export default ({ api, networks }) => {
  ...
  const generateNextReceiveAddress = function* (action) {
    ...
    try {
      ...
      const receiveIndex = yield select(selectors.core.data.btc.getReceiveIndex(account.xpub))
  ...
  const fetchUnusedAddresses = function* (action) {
    ...
    try {
      ...
      const labeledAddrs = labels.map((la) => ({
        address: Types.HDAccount.getReceiveAddress(account, la.index, networks.btc),
        index: la.index,
        label: la.label,
      }))
  ...
  const fetchUsedAddresses = function* (action) {
    ...
    try {
      ...
      const derivedAddrs = yield call(deriveAddresses, account, receiveIndex.getOrElse(0))
      // fetch blockchain data for each address
