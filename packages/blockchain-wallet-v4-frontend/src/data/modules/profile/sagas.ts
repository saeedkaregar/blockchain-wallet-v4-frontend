...
export default ({ api, coreSagas, networks }) => {
  ...
  const renewSession = function* (userId, lifetimeToken, email, guid, renewIn = 0) {
    try {
      yield delay(renewIn)
  ...
  const shareWalletAddressesWithExchange = function* () {
    try {
      ...
      const BTC = selectors.core.common.btc.getNextAvailableReceiveAddress(networks.btc, defaultIdx)
      // BCH
