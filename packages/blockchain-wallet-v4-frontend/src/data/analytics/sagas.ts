...
export default () => {
  ...
  const initUserSession = function* () {
    try {
      ...
      const paxBalance = (yield select(selectors.core.data.eth.getErc20Balance, 'pax')).getOrElse(0)
      const usdtBalance = (yield select(selectors.core.data.eth.getErc20Balance, 'usdt')).getOrElse(
        0
      )
