...
export const renderBankText = (value: SBPaymentMethodType | BankTransferAccountType): string => {
...
export const renderBank = (value: SBPaymentMethodType | BankTransferAccountType) => (
  <>
    ...
    <DisplayTitle>
      {`${value.details?.bankAccountType.toLowerCase()} account ${value.details?.accountNumber}`}
    </DisplayTitle>
  </>
)
...
export const getIcon = (
  ...
  switch (method.type) {
    ...
    case 'FUNDS':
      return (
        <Icon size='32px' color='fiat' name={method.currency.toLowerCase() as keyof IcoMoonType} />
      )
    case 'BANK_TRANSFER':
