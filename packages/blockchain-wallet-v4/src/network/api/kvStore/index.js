...
const toTask = (promise) => new Task((reject, resolve) => promise.then(resolve, reject))

export default ({ apiUrl, get, networks, put }) => {
  const updateKVStore = (kv) => {
    const createEncPayloadBuffer = kv.encKeyBuffer
      ? compose(KV.B64ToBuffer, KV.encrypt(kv.encKeyBuffer), JSON.stringify)
      : compose(KV.StringToBuffer, JSON.stringify)

    const encPayloadBuffer = createEncPayloadBuffer(kv.value)
    const signatureBuffer = KV.computeSignature(
      kv.signKey,
      encPayloadBuffer,
      kv.magicHash,
      networks.btc
    ...
    const request = put({
      ...
      data: body,
      transformRequest: compose(JSON.stringify, dissoc('api_code'), dissoc('ct')),
    }).catch(parseError)
    return toTask(request).map((res) => {
      const magicHash = KV.magic(encPayloadBuffer, kv.magicHash, networks.btc)
      return set(KV.magicHash, magicHash, kv)
    })
  }

  const fetchKVStore = (kv) => {
    const setKvFromResponse = curry((currentKv, res) => {
      if (res === null) return set(KV.value, null, currentKv)
      const setFromResponse = compose(
