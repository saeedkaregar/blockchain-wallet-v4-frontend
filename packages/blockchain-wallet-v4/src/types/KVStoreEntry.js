...
export const deriveMetadataNode = (masterHDNode) => {
  ...
  const hash = crypto.sha256('info.blockchain.metadata')
  const purpose = hash.slice(0, 4).readUInt32BE(0) & 0x7fffffff // 510742
...
export const magic = curry((payload, prevMagic, network) => {
  const msg = message(payload, prevMagic)
...
export const verifyResponse = curry((address, network, res) => {
  ...
  const verified = verify(address, sB, message(pB, mB), network)
  if (!verified) {
