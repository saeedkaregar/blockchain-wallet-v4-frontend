...
export const upgradeToHd = curry((mnemonic, firstLabel, password, network, wallet) => {
  return newHDWallet(mnemonic, password, wallet).chain(newHDAccount(firstLabel, password, network))
})
...
export const traverseKeyValues = curry((of, f, wallet) => {
  ...
  const trXpriv = traverseOf(
    compose(hdWallets, traversed, HDWallet.accounts, traversed, HDAccount.xpriv),
    of,
    f
  )
...
export const getHDPrivateKeyWIF = curry((keypath, secondPassword, network, wallet) => {
  const [accId, chain, index] = map(parseInt, split('/', keypath))
